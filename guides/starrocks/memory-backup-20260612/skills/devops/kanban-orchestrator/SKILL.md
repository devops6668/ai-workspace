...
## Kanban Worker Reference

For worker-side details (workspace handling, claim shapes, block reasons, heartbeats, retries, notification routing, CLI fallback): see `references/worker-guide.md`.

The worker skill covers the same topics from the worker's perspective. Key concepts that apply to both orchestrator and worker:

- `kanban_complete(summary=..., metadata=...)` — handoff shapes (coding, research, review)
- `kanban_block(reason=...)` + `kanban_comment()` — good blocking patterns
- `created_cards` gate — only claim ids from your `kanban_create` returns
- `goal_mode` cards — persistent workers with judge re-check
- Retry diagnostics — timed_out, crashed, spawn_failed, reclaimed, blocked
