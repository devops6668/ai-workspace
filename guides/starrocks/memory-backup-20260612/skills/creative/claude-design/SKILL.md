...
metadata:
  hermes:
    ...
    categories: [web-design, ui-prototyping, creative]
---
...
**Before starting, check for `design-md` (Google's DESIGN.md token spec format).** If the deliverable is a formal token spec file, use `design-md` instead. Full decision table below.

## 54 Design Templates

This skill includes 54 ready-to-paste design system templates from real products
(Claude, Stripe, Linear, Vercel, Apple, Spotify, Airbnb, etc.). Each template
captures the complete visual language: color palette, typography hierarchy,
component styles, spacing system, shadows, responsive behavior, and Hermes
implementation notes with CDN font substitutes.

To use a template:
```
skill_view(name="claude-design", file_path="templates/<site>.md")
```

Common mappings:
- **Developer tools / dashboards:** linear.app, vercel, supabase, raycast, sentry, cursor
- **Documentation / content:** mintlify, notion, sanity, mongodb
- **Marketing / landing pages:** stripe, framer, apple, spacex
- **Dark mode UIs:** linear.app, cursor, elevenlabs, warp, superhuman
- **Light / clean UIs:** vercel, stripe, notion, cal, replicate
- **Playful / friendly:** posthog, figma, lovable, zapier, miro
- **Premium / luxury:** apple, bmw, stripe, superhuman, revolut

---

## UI Sketching (Throwaway Mockups)

For throwaway UI mockups to explore design directions before committing, use the
`sketch` workflow: produce 2-3 interactive HTML variants and compare them head-to-head.

Use this when the user says "sketch this screen", "show me what X could look like",
"compare layout A vs B", "give me 2-3 takes on this UI", "let me see some variants".

**When NOT to use:** User wants a polished artifact → use claude-design directly.
User wants a diagram → use excalidraw or architecture-diagram.

### Workflow

1. **Intake:** Ask for feel, references, and core action (one question at a time)
2. **Variants:** Build 2-3 distinct HTML variants (different design stance, not just color swaps)
   - Each variant: single self-contained HTML with inline CSS
   - Interactive: links clickable, state transitions, hover states
3. **Verify:** Open in browser with `browser_vision` to catch layout bugs
4. **Compare:** Present as head-to-head comparison table, opinionate
5. **Consolidate:** Let user pick a winner or combine into hybrid
