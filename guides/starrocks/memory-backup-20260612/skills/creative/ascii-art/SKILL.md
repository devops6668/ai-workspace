...
## Tool 10: ASCII Video (Animated ASCII Art)

For animated ASCII art — video-to-ASCII, audio-reactive visualizers, generative ASCII animations, and hybrid video+audio projects. See `references/` for full pipeline docs.

### When to Use

- User asks for ASCII video, ASCII animation, terminal-style video, matrix effects, retro text animation, audio visualizer in ASCII, converting video to ASCII art
- NOT for static ASCII art — use Tools 1-9 above
...
| TTS narration | Text quotes + TTS API | Narrated video with typed text |

### Stack

- Python 3.10+, NumPy, SciPy, Pillow (PIL), ffmpeg, OpenCV (optional)
- Single self-contained Python script per project
...
| File | Purpose |
|------|---------|
| `references/architecture.md` | Grid system, character palettes, color system, resolution presets |
| `references/composition.md` | Pixel blend modes, tonemapping, feedback buffers, masking |
| `references/effects.md` | Effect building blocks: noise, particles, SDFs, attractors |
| `references/shaders.md` | Shader chain, transitions, tint presets, output encoding |
| `references/scenes.md` | Scene protocol, renderers, beat-synced cutting |
| `references/inputs.md` | Audio FFT, video sampling, image conversion, TTS |
| `references/optimization.md` | Hardware detection, quality profiles, parallel rendering |
| `references/troubleshooting.md` | Common traps: NumPy broadcasting, ffmpeg deadlocks, font issues |

### Critical Notes

- **Never use `canvas * N` for brightness** — use the adaptive `tonemap()` function (see `references/composition.md`)
- **Never `stderr=subprocess.PIPE` with long-running ffmpeg** — redirect to file to avoid 64KB deadlock
- **Validate Unicode palettes at init** — not all chars render in all fonts
- **Creative standard**: ASCII characters are the medium; cinema is the standard. First-render excellence is non-negotiable.
...
8. **Animated ASCII art / ASCII video** → ASCII video pipeline (Tool 10)
